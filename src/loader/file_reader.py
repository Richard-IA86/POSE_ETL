...
class FileReader:
    """Busca y lee los archivos crudos depositados en el Buzón Único
    (data/inbox).
    """
    ...
    def buscar_archivos(self, patron: str = "*.xlsx") -> list[Path]:
        """Devuelve archivos en el inbox que coincidan con el patrón."""
        esperados = list(self.inbox_dir.glob(patron))
        logging.info(
            f"{len(esperados)} archivos con patrón '{patron}' "
            f"en {self.inbox_dir}."
        )
        return esperados
